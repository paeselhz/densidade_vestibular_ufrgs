...
class DensitiesUFRGS:
    ...
    def run_bs4(self):
        ...
        self.table_courses = tabelas_cursos_div.find_all(
            "table", class_="col s12 curso"
        )
    ...
    def parse_table(self, bs4_table):
        ...
        df = pd.DataFrame(
            table_data, columns=["categoria", "n_inscritos", "n_vagas", "drop"]
        )
        df.drop("drop", axis=1)
        ...
        df = pd.concat(
            [
                df,
                pd.DataFrame(
                    [["Total", df["n_inscritos"].sum(), df["n_vagas"].sum()]],
                    columns=["categoria", "n_inscritos", "n_vagas"],
                ),
            ]
        )
    ...
    def export_parquet(self, folder="data"):
        ...
        self.all_years = self.all_years[
            ["ano", "curso", "categoria", "n_inscritos", "n_vagas", "densidade"]
        ]
